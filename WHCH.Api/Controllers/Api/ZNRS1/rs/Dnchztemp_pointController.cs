...
using WHCH.Api.RequestPayload.Rbac.Hztemp_point;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class Dnchztemp_pointController : ControllerBase
    {
        ...
        public Dnchztemp_pointController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnchztemp_point.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(Dnchztemp_pointRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dnchztemp_point.AsQueryable();
                //模糊查询
                ...
                {
                    query = query.Where(x => x.DncType_Name.Contains(payload.Kw));
                }
                ...
                var data = list.Select(_mapper.Map< Dnchztemp_point, Dnchztemp_pointJsonModel>);
        ...
        public IActionResult Create(Dnchztemp_pointCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< Dnchztemp_pointCreateViewModel, Dnchztemp_point>(model);
                ...
                _dbContext.Dnchztemp_point.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnchztemp_point.FirstOrDefault(x => x.Id == int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnchztemp_point, Dnchztemp_pointCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(Dnchztemp_pointEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnchztemp_point.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Dnchztemp_point SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE Dnchztemp_point SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< Dnchztemp_pointCreateViewModel>> res = ValidateJson.Validation< Dnchztemp_pointCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< Dnchztemp_pointCreateViewModel> arr = res.Value;
                            foreach ( Dnchztemp_pointCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< Dnchztemp_pointCreateViewModel, Dnchztemp_point>(item);
