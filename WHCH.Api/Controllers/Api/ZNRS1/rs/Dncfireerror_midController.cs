...
using WHCH.Api.RequestPayload.Rbac.Fireerror_mid;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class Dncfireerror_midController : ControllerBase
    {
        ...
        public Dncfireerror_midController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dncfireerror_mid.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(Dncfireerror_midRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dncfireerror_mid.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dncfireerror_mid, Dncfireerror_midJsonModel>);
        ...
        public IActionResult Create(Dncfireerror_midCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< Dncfireerror_midCreateViewModel, Dncfireerror_mid>(model);
                ...
                _dbContext.Dncfireerror_mid.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dncfireerror_mid.FirstOrDefault(x => (x.Id+"") == code);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dncfireerror_mid, Dncfireerror_midCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(Dncfireerror_midEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dncfireerror_mid.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Dncfireerror_mid SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE Dncfireerror_mid SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< Dncfireerror_midCreateViewModel>> res = ValidateJson.Validation< Dncfireerror_midCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< Dncfireerror_midCreateViewModel> arr = res.Value;
                            foreach ( Dncfireerror_midCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< Dncfireerror_midCreateViewModel, Dncfireerror_mid>(item);
