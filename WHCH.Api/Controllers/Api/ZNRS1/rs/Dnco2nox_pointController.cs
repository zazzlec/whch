...
using WHCH.Api.RequestPayload.Rbac.O2nox_point;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class Dnco2nox_pointController : ControllerBase
    {
        ...
        public Dnco2nox_pointController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnco2nox_point.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(Dnco2nox_pointRequestPayload payload)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var query = _dbContext.Dnco2nox_point.AsQueryable();
                    //模糊查询
                    ...
                    var data = list.Select(_mapper.Map<Dnco2nox_point, Dnco2nox_pointJsonModel>);
        ...
        public IActionResult Create(Dnco2nox_pointCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< Dnco2nox_pointCreateViewModel, Dnco2nox_point>(model);
                ...
                _dbContext.Dnco2nox_point.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnco2nox_point.FirstOrDefault(x => (x.Id+"") == code);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnco2nox_point, Dnco2nox_pointCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(Dnco2nox_pointEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnco2nox_point.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Dnco2nox_point SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                var sql = string.Format("UPDATE Dnco2nox_point SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< Dnco2nox_pointCreateViewModel>> res = ValidateJson.Validation< Dnco2nox_pointCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< Dnco2nox_pointCreateViewModel> arr = res.Value;
                            foreach ( Dnco2nox_pointCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< Dnco2nox_pointCreateViewModel, Dnco2nox_point>(item);
