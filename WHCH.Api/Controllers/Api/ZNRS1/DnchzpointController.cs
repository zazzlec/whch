...
using WHCH.Api.RequestPayload.Rbac.Hzpoint;
using WHCH.Api.ViewModels.Rbac.Dnchzpoint;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DnchzpointController : ControllerBase
    {
        ...
        public DnchzpointController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnchzpoint.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DnchzpointRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dnchzpoint.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dnchzpoint, DnchzpointJsonModel>);
        ...
        public IActionResult Create(DnchzpointCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DnchzpointCreateViewModel, Dnchzpoint>(model);
                ...
                _dbContext.Dnchzpoint.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnchzpoint.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnchzpoint, DnchzpointCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DnchzpointEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnchzpoint.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dnchzpoint  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dnchzpoint SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dnchzpoint SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DnchzpointCreateViewModel>> res = ValidateJson.Validation< DnchzpointCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DnchzpointCreateViewModel> arr = res.Value;
                            foreach ( DnchzpointCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DnchzpointCreateViewModel, Dnchzpoint>(item);
