...
using WHCH.Api.RequestPayload.Rbac.Pointkks_data;
using WHCH.Api.ViewModels.Rbac.Dncpointkks_data;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class Dncpointkks_dataController : ControllerBase
    {
        ...
        public Dncpointkks_dataController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dncpointkks_data.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(Dncpointkks_dataRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dncpointkks_data.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dncpointkks_data, Dncpointkks_dataJsonModel>);
        ...
        public IActionResult Create(Dncpointkks_dataCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< Dncpointkks_dataCreateViewModel, Dncpointkks_data>(model);
                ...
                _dbContext.Dncpointkks_data.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dncpointkks_data.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dncpointkks_data, Dncpointkks_dataCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(Dncpointkks_dataEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dncpointkks_data.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dncpointkks_data  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dncpointkks_data SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dncpointkks_data SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< Dncpointkks_dataCreateViewModel>> res = ValidateJson.Validation< Dncpointkks_dataCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< Dncpointkks_dataCreateViewModel> arr = res.Value;
                            foreach ( Dncpointkks_dataCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< Dncpointkks_dataCreateViewModel, Dncpointkks_data>(item);
