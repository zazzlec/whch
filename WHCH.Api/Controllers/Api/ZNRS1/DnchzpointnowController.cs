...
using WHCH.Api.RequestPayload.Rbac.Hzpointnow;
using WHCH.Api.ViewModels.Rbac.Dnchzpointnow;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DnchzpointnowController : ControllerBase
    {
        ...
        public DnchzpointnowController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnchzpointnow.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DnchzpointnowRequestPayload payload)
        {
            ...
            using (_dbContext)
            {
                ...
                var data = list.Select(_mapper.Map< Dnchzpointnow, DnchzpointnowJsonModel>);
        ...
        public IActionResult Create(DnchzpointnowCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DnchzpointnowCreateViewModel, Dnchzpointnow>(model);
                ...
                _dbContext.Dnchzpointnow.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnchzpointnow.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnchzpointnow, DnchzpointnowCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DnchzpointnowEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnchzpointnow.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dnchzpointnow  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dnchzpointnow SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dnchzpointnow SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DnchzpointnowCreateViewModel>> res = ValidateJson.Validation< DnchzpointnowCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DnchzpointnowCreateViewModel> arr = res.Value;
                            foreach ( DnchzpointnowCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DnchzpointnowCreateViewModel, Dnchzpointnow>(item);
