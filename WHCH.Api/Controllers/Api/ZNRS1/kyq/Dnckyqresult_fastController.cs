...
using WHCH.Api.RequestPayload.Rbac.Kyqresult_fast;
using WHCH.Api.ViewModels.Rbac.Dnckyqresult_fast;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class Dnckyqresult_fastController : ControllerBase
    {
        ...
        public Dnckyqresult_fastController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnckyqresult_fast.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(Dnckyqresult_fastRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dnckyqresult_fast.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dnckyqresult_fast, Dnckyqresult_fastJsonModel>);
        ...
        public IActionResult Create(Dnckyqresult_fastCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< Dnckyqresult_fastCreateViewModel, Dnckyqresult_fast>(model);
                ...
                _dbContext.Dnckyqresult_fast.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnckyqresult_fast.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnckyqresult_fast, Dnckyqresult_fastCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(Dnckyqresult_fastEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnckyqresult_fast.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql = string.Format("UPDATE Dnckyqresult_fast SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dnckyqresult_fast SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< Dnckyqresult_fastCreateViewModel>> res = ValidateJson.Validation< Dnckyqresult_fastCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< Dnckyqresult_fastCreateViewModel> arr = res.Value;
                            foreach ( Dnckyqresult_fastCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< Dnckyqresult_fastCreateViewModel, Dnckyqresult_fast>(item);
