...
using WHCH.Api.RequestPayload.Rbac.Kyqpoint;
using WHCH.Api.ViewModels.Rbac.Dnckyqpoint;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DnckyqpointController : ControllerBase
    {
        ...
        public DnckyqpointController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnckyqpoint.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DnckyqpointRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dnckyqpoint.AsQueryable();
                //模糊查询
                ...
                if (payload.tid != -1)
                {
                    query = query.Where(x => x.DncKyqTpyeId == payload.tid);
                }
                ...
                var data = list.Select(_mapper.Map< Dnckyqpoint, DnckyqpointJsonModel>);
        ...
        public IActionResult Create(DnckyqpointCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DnckyqpointCreateViewModel, Dnckyqpoint>(model);
                ...
                _dbContext.Dnckyqpoint.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnckyqpoint.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnckyqpoint, DnckyqpointCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DnckyqpointEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnckyqpoint.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql = string.Format("UPDATE Dnckyqpoint SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dnckyqpoint SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DnckyqpointCreateViewModel>> res = ValidateJson.Validation< DnckyqpointCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DnckyqpointCreateViewModel> arr = res.Value;
                            foreach ( DnckyqpointCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DnckyqpointCreateViewModel, Dnckyqpoint>(item);
