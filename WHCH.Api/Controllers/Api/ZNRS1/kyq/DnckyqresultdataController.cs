...
using WHCH.Api.RequestPayload.Rbac.Kyqresultdata;
using WHCH.Api.ViewModels.Rbac.Dnckyqresultdata;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DnckyqresultdataController : ControllerBase
    {
        ...
        public DnckyqresultdataController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dnckyqresultdata.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DnckyqresultdataRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dnckyqresultdata.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dnckyqresultdata, DnckyqresultdataJsonModel>);
        ...
        public IActionResult Create(DnckyqresultdataCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DnckyqresultdataCreateViewModel, Dnckyqresultdata>(model);
                ...
                _dbContext.Dnckyqresultdata.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dnckyqresultdata.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dnckyqresultdata, DnckyqresultdataCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DnckyqresultdataEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dnckyqresultdata.FirstOrDefault(x => x.Id == model.Id);
                ...
                entity.Zczc_Oil_Temp_Val = model.Zczc_Oil_Temp_Val;
                entity.Dxzc_Oil_Temp_Val = model.Dxzc_Oil_Temp_Val;
                entity.Zczc_Oil_Dif_Val = model.Zczc_Oil_Dif_Val;
                entity.Dxzc_Oil_Dif_Val = model.Dxzc_Oil_Dif_Val;
                entity.Air_Temp_Out_Val = model.Air_Temp_Out_Val;
                entity.Air_Temp_In_Val = model.Air_Temp_In_Val;
                entity.Gas_Temp_In_Val = model.Gas_Temp_In_Val;
                entity.Air_Radio_Val = model.Air_Radio_Val;
                entity.Gas_Radio_Val = model.Gas_Radio_Val;
                entity.DncKyq = _dbContext.Dnckyq.FirstOrDefault(x => x.K_Name_kw == model.DncKyq_Name);
                entity.DncKyq_Name = entity.DncKyq.K_Name_kw;
                entity.DncKyq_Name = model.DncKyq_Name;
                entity.Gas_Temp_Out_Val = model.Gas_Temp_Out_Val;
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql = string.Format("UPDATE Dnckyqresultdata SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dnckyqresultdata SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DnckyqresultdataCreateViewModel>> res = ValidateJson.Validation< DnckyqresultdataCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DnckyqresultdataCreateViewModel> arr = res.Value;
                            foreach ( DnckyqresultdataCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DnckyqresultdataCreateViewModel, Dnckyqresultdata>(item);
