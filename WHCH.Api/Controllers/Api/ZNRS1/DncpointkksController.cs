...
using WHCH.Api.RequestPayload.Rbac.Pointkks;
using WHCH.Api.ViewModels.Rbac.Dncpointkks;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DncpointkksController : ControllerBase
    {
        ...
        public DncpointkksController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dncpointkks.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DncpointkksRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dncpointkks.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dncpointkks, DncpointkksJsonModel>);
        ...
        public IActionResult Create(DncpointkksCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DncpointkksCreateViewModel, Dncpointkks>(model);
                ...
                _dbContext.Dncpointkks.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dncpointkks.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dncpointkks, DncpointkksCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DncpointkksEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dncpointkks.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dncpointkks  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dncpointkks SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dncpointkks SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DncpointkksCreateViewModel>> res = ValidateJson.Validation< DncpointkksCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DncpointkksCreateViewModel> arr = res.Value;
                            foreach ( DncpointkksCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DncpointkksCreateViewModel, Dncpointkks>(item);
