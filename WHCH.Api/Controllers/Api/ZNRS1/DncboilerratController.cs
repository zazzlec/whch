...
using WHCH.Api.RequestPayload.Rbac.Boilerrat;
using WHCH.Api.ViewModels.Rbac.Dncboilerrat;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DncboilerratController : ControllerBase
    {
        ...
        public DncboilerratController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dncboilerrat.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DncboilerratRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dncboilerrat.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dncboilerrat, DncboilerratJsonModel>);
        ...
        public IActionResult Create(DncboilerratCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DncboilerratCreateViewModel, Dncboilerrat>(model);
                ...
                _dbContext.Dncboilerrat.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dncboilerrat.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dncboilerrat, DncboilerratCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DncboilerratEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dncboilerrat.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dncboilerrat  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dncboilerrat SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dncboilerrat SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DncboilerratCreateViewModel>> res = ValidateJson.Validation< DncboilerratCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DncboilerratCreateViewModel> arr = res.Value;
                            foreach ( DncboilerratCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DncboilerratCreateViewModel, Dncboilerrat>(item);
