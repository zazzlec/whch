...
using WHCH.Api.RequestPayload.Rbac.Chqpoint;
using WHCH.Api.ViewModels.Rbac.Dncchqpoint;
...
namespace WHCH.Api.Controllers.Api.WHCH1
{
    ...
    public class DncchqpointController : ControllerBase
    {
        ...
        public DncchqpointController(WHCHDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult Create(DncchqpointCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                if (string.IsNullOrEmpty(model.DncCharea_Name))
                {
                    entity.DncCharea = _dbContext.Dnccharea.FirstOrDefault(x => (x.Name_kw + "") == "无");
                    entity.DncCharea_Name = entity.DncCharea.Name_kw;
                }
                else
                {
                    entity.DncCharea = _dbContext.Dnccharea.FirstOrDefault(x => (x.Name_kw + "") == model.DncCharea_Name);
                    entity.DncCharea_Name = entity.DncCharea.Name_kw;
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dncchqpoint  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    ...
                    using (_dbContext)
                    {
                        ...
                        if (res.Key.Equals("ok"))
                        {
                            ...
                            foreach ( DncchqpointCreateViewModel item in arr)
                            {
                                ...
                                entity.DncCharea = _dbContext.Dnccharea.FirstOrDefault(x => x.Name_kw == item.DncCharea_Name);
